    ...
    def _send_reply(self, context: Context, reply: Reply):
        if reply and reply.type:
            ...
            if getattr(_plugin_action, "name", "") == "BREAK" or str(_plugin_action).endswith("BREAK"):
                logger.info("[ChatChannel] reply sending cancelled by plugin")
                return
            try:
                _ctx_kwargs = getattr(context, "kwargs", {}) or {}
                if _ctx_kwargs.get("split_reply_sent"):
                    logger.info("[ChatChannel] original reply cancelled by SplitReply")
                    return
            except Exception:
                pass
            if not e_context.is_pass() and reply and reply.type:
