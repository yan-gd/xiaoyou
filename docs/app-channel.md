...
- Android 通知权限由原生 Activity 查询和请求；授权失败或不再弹出权限框时，客户端会提供系统通知设置入口，
  并在用户返回后按真实系统权限自动完成 App 内开关。权限调用异常不会阻塞设置页。
- 客户端区分“系统权限状态”和“用户在 App 内明确关闭”：旧版本失败残留的关闭状态会按系统权限迁移修复，
  但用户主动关闭的通知不会被自动重新启用。
