...
- 小悠回复使用火山引擎 Seed-TTS 2.0 与
  `zh_female_xiaohe_uranus_bigtts`（小荷 2.0）合成。
...
XIAOYOU_APP_TTS_VOICE=zh_female_xiaohe_uranus_bigtts
```

新版火山语音控制台只需配置 `XIAOYOU_APP_TTS_API_KEY`。若账号仍使用
旧版鉴权，则留空该变量并同时配置 `XIAOYOU_APP_TTS_APP_ID` 与
`XIAOYOU_APP_TTS_ACCESS_KEY`。凭证只放服务器 `.env`，不要写入 APK
或提交 Git。
...
```text
[AppChannel] inited ... voice=True asr=qwen3-asr-flash tts=seed-tts-2.0 provider=volcengine tts_ready=True
```
...
- 语音回复会增加一次火山 Seed-TTS 2.0 调用；V3 接口返回 MP3，
  TTS 完成后 App 才收到可播放事件。
