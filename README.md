...
`XiaoyouLifePhoto v0.7` 使用 Qwen 结合人格、时间和记忆一次性规划画面，再通过 Seedream 生成人物一致的生活照。参考图和身体设定只影响图片生成。

- 四张中性参考图分别约束正面脸、左右四分之三脸和全身体态；旧婚纱图不再作为主动参考，避免继承固定服装和表情。
- “报备、分享生活、展示自己”不锁定镜头：模型可自主选择前置自拍、镜子自拍、定时拍摄、第三人称拍摄或第一视角场景。
- 第三人称、他拍、双手动作、完整全身和复杂 pose 会先经过本地物理规则校正，避免双手托脸却被规划成前置自拍。
- 情绪、眉眼嘴型、视线、动作和随图文字由同一次语义规划共同决定，并参考近期记录减少固定卖萌表情和姿势重复。
- 不增加生成后的视觉质检或第二次模型请求，因此不会因本次优化额外增加一次模型延迟。

### 主动行为

- `ConversationFollowup`：聊天停顿后的分钟级自然续聊。正常语境下即使分类模型认为“轮到对方回复”，仍保留一次约 4 分钟后的温和续聊机会；你提前回复会立即取消。明确要求安静、睡觉、开会、考试、驾驶、分类请求失败或生成模型选择 `NO_MESSAGE` 时不会发送。每轮最多跟进一次。
- `ProactiveLove`：长周期生活感主动联系，当前空闲阈值约 2 小时，与分钟级续聊相互独立
...
- 长期记忆增加时间感知重排
- 新增聊天中断追问和模型中断重连
- 生活照升级为四参考图、语义表情、第三人称镜头和零额外模型调用的物理规则校正
- ConversationFollowup 增加约 4 分钟的一次性温和续聊，同时保留明确免打扰和新消息取消机制
