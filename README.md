...
`ContextPlanner` 是不调用模型的本地查询规划器。它把本轮分为纠正、明确回忆、偏好、项目、情绪承接、短句续聊或一般聊天，并分别决定本地情节和云端长期记忆的候选数。情绪承接只使用眼前上下文；短句最多用一个相关本地情节兜底但不查云端；回忆、项目和纠正才扩大相关历史。这样不会为了“显得有记忆”而每轮固定塞满记忆。
...
RecentState 位于 `/app/data/xiaoyou_recent_state/state.json`，只保存带逐字证据和过期时间的当前话题、YoYo临时状态、小悠承接立场、未完事项、指代及当前时段事实。它在回复完成后后台串行更新，不阻塞当前发送，也不进入阿里云长期记忆。如果状态更新被内容审查拒绝，旧派生状态会暂时停止注入，避免过期场景压过准确的原生近期消息；下一次成功更新后自动恢复。
...
长期记忆正文存储在阿里云百炼记忆服务。读取时会结合语义相关度、明确时间表达、近期时间权重和查询规划器允许的记忆类型排序；本地只保存一份不参与直接回复的治理账本，用来审计候选证据、去重覆盖和云端写入结果。

受治理记忆分为情节记忆、语义记忆、关系记忆、项目记忆、未完事项和纠正记录；当前轮与近期原话属于工作记忆，不写成永久云端事实。旧百炼节点没有类型字段时按“旧版记忆”兼容召回，后续新写入和纠正会逐步带上规范类型，不需要一次性迁移或清库。
...
治理账本位于 `/app/data/xiaoyou_memory/memory_governance.json`，包含候选状态、用户证据摘要、云端记忆 ID、覆盖关系、失败原因和最近 500 条决策。提取模型或账本不可用时写入会失败关闭；默认禁止退回旧的“每轮用户原话 + 助手回复直接写入”方式。
...
用户原话
  ...
  → memory_key 去重或覆盖
  → 本地审计账本
  → 百炼 custom_content 新增或 PATCH 更新
```
...
- 长期记忆增加候选提取、用户证据校验、置信度与重要度门槛、本地审计账本
- 小悠回复不再进入长期记忆写入内容，模型提取失败时默认不写入
- 同一语义键的新事实会覆盖旧事实，并通过百炼 UpdateMemory 原地更新云端片段
