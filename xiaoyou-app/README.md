...
- 使用仓库根目录 `assets/applogo.png` 生成 Android 与 iOS 的标准尺寸应用图标
...
Android 13 及以上首次开启通知时会通过原生权限接口请求系统授权；若系统不再显示授权弹窗，App 会提供直接
进入“小悠”通知设置的入口。从系统设置返回后会重新读取真实权限，并在授权成功时自动完成 App 内通知开关。
通知插件初始化、权限请求和设置跳转均有超时与异常收尾，不会让设置开关永久停在加载状态。
